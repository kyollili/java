 ...
 *  						한글(한 글자당 => 2byte)
 *  		논리
 *  	***참조형(프로그래머가 직접 메모리 크기 제작) => 기본형 여러 개 묶어서 사용
 *  		1) 배열 : 같은 데이터만 묶어서 관리
 ...
 *  		=> 맨 앞에는 사용 금지
 *  	3) 특수문자 사용이 가능(_,$)
 *  	4) 키워드는 사용 금지(자바에서 지원하는 단어)
 *  		if, for, while, do, int, double,,,,
 *  	5) 변수의 길이는 제한하지 않는다 (보통은 3~7자리)
 *  	-------> 권장사항 	클래스명은 대문자 시작 String, System,,
 *  			 		변수, 메소드 명은 소문자 시작
 *  			 		상수는 전체를 대문자 			--------> 분석이 쉽다 => 리펙토링
 *  	
 *  	=> 변수 선언, 상수 선언
 *  	데이터형 변수명 ------------------> 선언=>초기화, 선언과 초기값(변경이 가능)
 *  	final 데이터형 변수명 ------------> 선언과 동시에 초기화 (변경할 수 없다)
 *  
 *  	*** 변수 선언(국어, 수학, 영어)
 *  	int kor;
 *  	int math;
 ...
 *  			  -----> 자동으로 long
 *  		int a ='A'; ==> 65
 *  		int a=10.5; ==> 저장이 불가능 (int가 10.5보다 작기때문)
 *  		
 *  		=>연산처리(연산=> 항상 같은 데이터형만 연산이 가능=>큰 데이터형으로 변환)
 *  		10+10.5 ==>10.0+10.5 =>20.5
 *  		'A'+1==>65+1 ==>66
 *  
 *  		형변환 : (int)"10" ==> 문자열은 형변환 불가능
 *  		*** 프로그램에 맞게 데이터형을 변경해서 사용할 수 있다(기본형, 클래스형)
 *  		p.82-83 ==> 자동형변환
 *  		*** int 이하 데이터형(byte,short,char) ==>연산처리가 되면 결과값 int
 *  		----크거나 같아야함
 */
public class 변수_p20 {
