 ...
 * 		런타입에러 : 실행시 에러 java
 * 		논리적에러 : 실행은 되지만 프로그램 제작과 다르게 동작
 * 		--------------------------------------------------
 * 		견고한 프로그램 : 에러가 발생 시 종료하지 않고 정상 수행이 가능하게 만드는 것
 ...
 * 			웹 : 404, 500, 415, 400
 * 				-------------------
 * 				404 => 파일이 없는 경우 (웹=>서버에 파일을 보여달라) => 파일명을 변경
 * 				500 => 문법 에러
 * 				415 => 한글변환(UTF-8)
 ...
 * 			= 예외 임의발생 : 테스팅 시 예외를 발생해서 견고한 프로그램인지 확인
 * 				throw 예외처리;
 ...
 * 			Error : 메모리 부족, 윈도우 작동 안함, 이클립스 문제 발생..
 * 			Exception : 파일명 틀리다, IP가 틀리다, 웹사이트주소가 틀리다 ...,SQL문장을 잘못 수행
 * 						Object
 * 							|
 ...
 * 			3. 예외 이믜 발생(사용자가 예외처리)	
 * 			4. 사용자 정의 예외처리	
 * 			
 * 			1. 직접처리(예외 복구) : 웹, 데이터베이스는 CheckException (무조건 예외처리를 해야한다)
 ...
 * 				}catch(예외처리 종류)
 * 				{
 * 					에러발생 처리하는 영역 => 에러 확인 => 예상되는 예외처리 ==> catch 여러번 사용 가능
 * 				}
 * 				finally
 * 				{
 * 					try,catch 수행 상관없이 무조건 수행하는 문장
 * 					에러발생, 정상수행 상관 없이 무조건 수행하는 문장이 존재
 * 					----------------------------------------
 * 						1) 파일 닫기
 * 						2) 오라클 닫기
 * 						3) 서버 닫기
 ...
 * 				***예외처리
 * 				1. 고민 : 예상되는 에러
 * 				예) 두 개의 정수를 문자열을 받아서 정수 배열에 저장 후 저장된 데이터를 나누기를 하는 프로그램을 제작
 * 										--------1.배열인덱스		3.-----
 * 										2.문자열=>정수
 * 				2. 고민 : 예외처리의 위치
 */
