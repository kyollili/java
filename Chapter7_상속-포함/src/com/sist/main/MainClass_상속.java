 ...
 * 			-----------------------------대부분은 인스턴스 변수가 기본이다
 * 		메소드 : 인스턴스변수, static 변수 활용 => 메소드 (제어문, 연산자)
 * 		생성자 : 메모리에 저장 시에 처음으로 호출되는 메소드 (무조건 한 개 이상을 포함한다)
 * 	객체지향 프로그램
 * 	------------
 * 		데이터보호 : 캡슐화
 * 		재사용 : 상속, 포함
 ...
 * 				class A extends B		B를 확장해서 새로운 A를 만들겠다
 * 					-----	  -----
 * 					하위클래스	  상위클래스
 * 					데이터형은 상속을 내리는 클래스(상위클래스)가 크다
 * 					메모리는 상속을 받는 클래스(하위클래스)가 크다
 * 			예)
 * 				class A
 * 				{
 * 					int a,b;
 ...
 * 							3) static => 사용은 가능
 * 					상속은 인스턴스만 상속을 내려준다
 * 					*** 클래스 자신만이 가지고 있는 것은 인스턴스 변수, 인스턴스메소드
 *  
 *   				상속 형식
...
class Sub extends Super
{
	 ...
	 * 		int a=10;
	 * 		int b=20;
	 * 		public void print()  ==> 변경해서 사용할 때는 오버라이딩이라고 함
	 * 		{
	 * 			System.out.println("a="+a);
				System.out.println("b="+b);
				System.out.println("c="+c);
 ...
 * 		------------메모리만 복제한다
 * 	복제 ==> 상위클래스가 먼저 메모리 할당 ==> 하위 클래스가 메모리 할당
 */
