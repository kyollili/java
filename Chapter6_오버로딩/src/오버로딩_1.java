 ...
 * 				옵션 ==> static , final
 * 		예)
 * 			int a; // 따로 저장 ==> 객체 생성 후 사용이 가능
 * 			static int b; // 공유 ==> 자동 저장 ==> 클래스명으로 접근이 가능(클래스명.변수명)
 * 			final int c=상수값 지정; // 상수 , 값을 변경하지 않음
 * 			--------------------------------------호출
 * 		- 클래스변수 : static 
 ...
 * 			검색하기	=> 메소드
 * 			---------------게시판 기능
 * 		메소드 => 리턴형 / 매개변수 (여러 개 설정) => Call By Value(메소드에서만 변경)
 * 											Call By Reference(배열, 클래스 자체 변경)
 * 				----한 개(배열,클래스)
 * 		* 인스턴스 메소드 => 인스턴스메소드 호풀, 인스턴스 변수, static 변수, static 메소드 => 사용이 가능
 * 		  static 메소드 => static만 호출이 가능(static 변수, static 메소드)
 * 						인스턴ㄴ스변수, 인스턴스메솓 사용 시에는 반드시 객체 생성 후에 접근이 가능
 * 	-----------------CBD
 * 	오버로딩 : 생성자 p.283
 * 		1) 하나의 메소드명으로 여러 개의 기능을 만들 수 있다(새로운 추가)
 * 		2) 메소드명을 줄일 수 있다
 * 			---------------기능이 동일
 * 			println(int a)
 * 			println(boolean b)
 ...
 * 				=> void display() : 원본
 * 				1. int display() : 오버로딩x
 * 				2. char display(int a) : 오버로딩o
 * 				3. void display(char c) : 오버로딩o
 * 				4. char display(int a, int b) : 오버로딩o
 * 				5. double display(double d1,double d2) : 오버로딩o
 * 				=>
...
public class 오버로딩_1 {
	...
	{
		System.out.println("display(char, int i, long l)Call...");
	}
