 ...
 * 				= 정적 변수(한 개만 저장하는 변수 => 데이터베이스(오라클)) ==> 공유(웹)
 * 			2) 메소드 : 한 가지 기능을 수행하는 명령문의 집합
 * 					  다른 클래스와 통신
 ...
 * 							int a=10;	(o)
 * 							int a;
 * 							a=10;		(x) //구현 => 연산자, 메소드 호출, 제어문 ==>초기화 블록{}, 메소드{}, 생성자{}
 * 						*** 초기화 블록은 자동 처리 , 상속예외조건
 * 		2. 클래스
 * 			1) 클래스 설계
 * 			2) 클래스 메모리에 저장 ==> 저장하는 과정 : 인스턴스화 ==> 저장된 상태(인스턴스,객체)
 * 				클래스명 객체=new 클래스명() ==> new 연산자는 메모리를 따로 생성
 * 			3) 활용 : 
 * 				객체.변수 , 객체.메소드() ....
 * 			4) 소멸
 * 				객체=null; => 주소값을 지운다 => GC
 ...
 * 			= 캡슐화
 * 				데이터 보호와 동시에 필요시에는 사용이 가능하게 만든다 (변수)
 * 				=> 변수 (읽기/쓰기)	==> getter/setter
 * 			= 상속/포함 : 재사용
 * 				상속 : 변경이 필요한 경우 => 게시판 => 갤러리
 * 				포함 : 변경 없이 사용
 * 				상송 => 예외(초기화블록, 생성자, static, private)
 * 						형식)
 * 							class A extends B
 * 								 ---	   ---
 * 								하위클래스	  상위클래스
 * 								B의 모든 내요을 상속 받아서 A라는 새로운 클래스를 만든다
 * 								=> 단일 상속만 가능하다
 * 							class A
 * 							{
 * 								B b=new B(); *******
 ...
 * 						** this는 생략이 가능 , 지역변수와 멤버변수가 동일할 때 구분 시 많이 사용
 * 						다른 클래스로 현재 사용하고 있는 객체를 전송할 때 사용 (윈도우=쓰레드 이용)
 * 				this() => 자신의 생성자를 호출할 때 사용 (생성자의 첫번째 줄에 사용)
 * 				super => 상속이 있는 경우 상위 클래스 객체
 * 						*** 상속을 받으면 하위클래스(상속을 받는 클래스) / 상위클래스(상속을 내리는 클래스)
 * 									  -------				  --------=> 하위클래스를 제어하지 못한다
 * 									=> 상위클래스의 변수나 메소드를 제어
 * 							상위 클래스 : this
 * 							하위 클래스 : this , super
 * 				super() => 상속 내린 클래스의 생성자 호출 (생성자 안에서 사용, 첫 줄에 사용)
 * 
 * 				예)
 * 					public class Parent
 * 					{
 ...
 * 					3) 생성자가 없는 경우 => 자동으로 생성자 추가 (디폴트 생성자 : 매개변수가 없는 생성자)
 * 						public class A
 * 						{
 * 							//public A(){} ==> 필요시에만 사용
 * 						}
 * 					4) 메소드는 반드시 return을 필요로 한다 (한 개 이상)
 * 						return => 메소드 종료 시점
 ...
 * 			-------- 추상클래스 / 인터페이스 ==> 관련된 클래스를 모아서 관리, 서로 관련 없는 클래스를 연결
 * 					 사용자 정의 클래스 관리(스프링)
 * 			public abstract class A : 추상클래스
 * 			public interface A : 추상 클래스의 일종 => 추상클래스의 단점을 보완
 * 												 -------- 단일 상속
 * 												 -------- 다중 상속
 ...
 * 			* 메소드 중에 private과 final을 동시에 사용할 수 없다
 * 
 * 			p.375 추상클래스 : 자신이 메모리 할당을 할 수 없다 , 미완성 된 클래스 => 사용시에는 구현된 클래스의 주소를 받아서 사용
 * 				=> class A
 * 				   class B extends A
 * 			
 * 				   B b=new B(); ==> 불가능
 ...
 * 				   B b1=(B)a;
 * 
 * 			추상클래스 => 요구사항분석(기능만 설정 => 구현은 나중에) ***미완성 (메소드가 구현이 안된 상태)
 * 			------- 기능이 여러가지인 경우에 설계 (프로그램에 맞게 구현해서 사용=> 구현 안됨)
 * 					예) 버튼, 마우스, 키보드
 * 					** 메모리 할당이 불가능하다 => 구현한 클래스를 대입해서 사용한다
 * 					** 데이터베이스의 모든 클래스는 추상클래스로 제작되어있다
 * 										   --------
 ...
 * 					--------------------------------------- 선언(구현(x)) => 사이트에 맞게 구현해서 사용한다
 * 					==> 오버라이딩 
 * 					==> 추상클래스 상속을 받아서 구현 후에 사용
 * 						-----------------
 * 						=> 관련된 클래스 여러 개 모아서 한 개의 이름으로 제어
 * 				
 */
abstract class 도형
{
	...
	/*
	 * 추상클래스는 
	 * 	= 클래스마다 따로 구현해야 하는 메소드(선언)
	 * 	= 필요시마다 구현해야 하는 메소드 (구현=>오버라이딩)
	 * 	= 멤버변수도 가지고 있다
	 * 	= 사용시에는 반드시 메소드르 구현한 클래스를 이용해서 메모리 할당을 한다
	 * 	= 관련된 클래스 여러 개를 묶어서 한 개의 이름으로 제어할 때 
	 */
}
class 사각형 extends 도형
{
	//상속 시에 구현이 안된 메소드는 반드시 구현해서 사용
	public void draw() //구현할 때는 abstract 제외
	{
...
public class MainClass_클래스종류 {
	...
	public static void main(String[] args) {
		...
		도형 graph=new 사각형(); //new 도형 하면 미완성이기 떄문에 오류
		graph.draw(); //생성자에 맞게 메소드 호출이 달라짐
