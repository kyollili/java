 ...
 * 				
 * 		= java.net : 흐름(역할 => 쓰레드, 내부클래스) => 서버 개발(웹서버가 이미 존재) => tomcat, 오라클서버
 ...
 */
/*
 * 		p.691 => 열거형 (데이터를 모아서 관리)
 * 					  ---------------- 관련된 일반 데이터 : 배열
 * 										관련된 상수 : 열거형 ==> 국번 (jdk 1.5 이상에서 실행)
 * 										==> JDK 1.8 => 호환성이 제일 좋음 => sun에서 마지막으로 만듦
 * 		=> STS / Eclipse / DBleaver / SqlDeveloper / Tomcat(9버전/10버전=>패키지명이 다르다)
 * 		=> 상수를 사용하기 쉽게 만드는 프로그램(데이터를 모아서 => 한 개의 이름으로 제어)
 * 		=> 사용법
 * 			enum 열거형명 {상수명, 상수명...} => 윈도우, 게임 개발, 서버 개발 ...
