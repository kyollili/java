 ...
 * 			1. 명시적 초기화 int a=10;
 * 						a=100; ====> 오류
 * 			class 영역에서는 외부나, 파일 읽기를 할 수 없다
 * 						-------------- 생성자, 초기화 블록
 * 			class 영역에서는 선언만 가능하다
 * 			---------메소드 호출, 값 대입, 제어문 (x)
 * 			-----------------------------------
 * 		}
 * 
 * 		생성자는 한 개만 있는 것이 아니라 경우에 따라서 여러 개 설정이 가능하다
 * 		생성자에는 생성자를 호출 할 수 있다
 * 		-------------------------------생성자에서 생성자 호출 ==> this()
 * 		this ==> 자신의 객체 ==> new를 이용해서 메모리가 할당될 때 자동으로 생성
 * 		------------------->멤버메소드에서만 사용이 가능
 * 							-------인스턴스 변수, 인스턴스메소드 (static은 this를 사용할 수 없다)
 * 							지역변수와 멤버변수 구분 할 때 주로 사용
 * 
 * 		1) 생성자
 * 			기본 생서자(디폴트 생성자) ==> () 매개변수가 없는 생성자
 ...
 * 				*** 매개변수를 가지고 있는 생성자가 한 개 이상 존재하면(컴파일러가 기본생성자를 첨부하지 않는다)
 * 							
 */
