 ...
 * 					= 단일 상속을 기본으로 한다
 * 					= 객체 생성하는 방법
 * 					  하위클래스 a=new 하위클래스() ==> 기본형식 (상위클래스를 확장) => 상속받은 클래스를 주로 이용한다
 * 						=> 상위클래스 a=new 상위클래스() ==> 사용빈도가 거의 없다
 * 						상위클래스 a=new 하위클래스() : 인터페이스, 추상클래스, 스프링, 데이터베이스
 * 								  ------------	 ----------------객체 생성을 할 수가 없다
 * 													(미완성 클래스 => 사용자가 완성해서 사용)
 * 												****인터페이스 (스프링의 기반) : 기술면접(95%)
 * 						*** 객체 생성시 => 생성자명이 다르다 (인터페이스 , 패턴 의심)
 ...
 * 		
 * 						==> 형변환이 가능 (상속, 포함)	
 * 			= 포함 (has-a) : 기존의 클래스를 포함해서 사용 => 변경 없이 사용시에 많이 사용한다
 * 
 ...
 * 				변경해서 사용 => 오버라이딩(덮어쓴다)
 * 					오버라이딩의 조건
 * 					1) 상속이 있어야한다(기존의 기능을 변경)
 * 					2) 메소드명이 동일하다
 * 					3) 매개변수가 동일하다(갯수, 데이터형..)_
 * 					4) 리턴형이 동일하다
